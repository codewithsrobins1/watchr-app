...
import { Settings, LogOut, Sun, Moon, Plus, Trash2, X, AlertTriangle } from 'lucide-react'
...
export default function Sidebar({
  ...
  // Delete confirmation
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'board' | 'community'; id: string; name: string } | null>(null)
  ...
  return (
    <>
      ...
      >
        {/* Logo */}
        <div className="p-4">
          <Image src="/logo.png" alt="Watchr" width={110} height={28} className="h-7 w-auto" />
        </div>
        ...
        <div className="flex-1 overflow-auto px-4 space-y-6">
          {/* My Boards */}
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>Your Boards</span>
            <div className="mt-2 space-y-1">
              {loading ? (
                <p className="text-sm px-3 py-2" style={{ color: theme.textMuted }}>Loading...</p>
              ) : myBoards.length > 0 ? (
                myBoards.map(board => (
                  <div key={board.id} className="group flex items-center">
                    <button
                      onClick={() => { setSelectedBoardId(board.id); setCurrentView('board') }}
                      className="flex-1 flex items-center gap-3 px-3 py-2 rounded-lg text-left transition-all"
                      style={{ 
                        backgroundColor: selectedBoardId === board.id && currentView === 'board' ? theme.accent.bg : 'transparent',
                        color: selectedBoardId === board.id && currentView === 'board' ? theme.accent.primary : theme.textSecondary
                      }}
                    >
                    ...
                    <button
                      onClick={() => setDeleteTarget({ type: 'board', id: board.id, name: board.name })}
                ...
                <p className="text-sm px-3 py-2" style={{ color: theme.textMuted }}>No boards yet</p>
              )}
            </div>
          </div>
          ...
          {sharedBoards.length > 0 && (
            <div>
              <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>Shared Boards</span>
              <div className="mt-2 space-y-1">
                {sharedBoards.map(board => (
                  ...
                  >
                    <span className="text-lg">{board.icon}</span>
                    <span className="flex-1 truncate text-sm font-medium">{board.name}</span>
                    <span className="text-xs px-1.5 py-0.5 rounded" style={{ backgroundColor: theme.bgTertiary, color: theme.textMuted }}>Shared</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {/* Communities */}
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>Community Feeds</span>
            <button
              onClick={() => setShowCreateCommunity(true)}
              className="w-full flex items-center gap-2 px-3 py-2 mt-2 rounded-lg font-medium transition-all"
              style={{ backgroundColor: theme.accent.bg, color: theme.accent.primary }}
            >
              <Plus className="w-4 h-4" />
            ...
            <div className="mt-1 space-y-1">
              {communities.map(comm => (
                <div key={comm.id} className="group flex items-center">
                  <button
                    onClick={() => { setSelectedCommunityId(comm.id); setCurrentView('community') }}
                    className="flex-1 flex items-center gap-3 px-3 py-2 rounded-lg text-left transition-all"
                    style={{ 
                      backgroundColor: selectedCommunityId === comm.id && currentView === 'community' ? theme.accent.bg : 'transparent',
                      color: selectedCommunityId === comm.id && currentView === 'community' ? theme.accent.primary : theme.textSecondary
                    }}
                  >
                    <span className="text-lg">{comm.icon}</span>
                    <span className="flex-1 truncate text-sm font-medium">{comm.name}</span>
                  </button>
                  {/* Only show delete for communities you own */}
                  {isOwner(comm) && (
                    <button
                      onClick={() => setDeleteTarget({ type: 'community', id: comm.id, name: comm.name })}
                      className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500/20"
        ...
        {/* Bottom */}
        <div className="p-4 space-y-2" style={{ borderTop: `1px solid ${theme.border}` }}>
          <button
            onClick={() => setCurrentView('settings')}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all"
            style={{ backgroundColor: currentView === 'settings' ? theme.accent.bg : 'transparent', color: currentView === 'settings' ? theme.accent.primary : theme.textSecondary }}
          >
          ...
          <div className="flex items-center justify-between px-3 py-2">
            ...
            >
              <div className="absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all" style={{ left: darkMode ? 'calc(100% - 20px)' : '4px' }} />
            </button>
          </div>

          <div className="flex items-center gap-3 px-3 py-2">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm" style={{ backgroundColor: theme.accent.bg, border: `2px solid ${theme.accent.primary}` }}>
              {profile?.avatar_emoji || '😎'}
            </div>
            <span className="flex-1 text-sm font-medium truncate" style={{ color: theme.text }}>{profile?.username || 'User'}</span>
            <button onClick={signOut} className="p-2 rounded-lg transition-all hover:bg-red-500/10" style={{ color: theme.textMuted }} title="Sign Out">
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setDeleteTarget(null)}>
          ...
          >
            ...
            <p className="mb-6" style={{ color: theme.textSecondary }}>
              Are you sure you want to delete <strong style={{ color: theme.text }}>{deleteTarget.name}</strong>? 
      ...
      {showCreateBoard && <CreateBoardModal onClose={() => setShowCreateBoard(false)} onCreated={handleBoardCreated} />}
      {showCreateCommunity && <CreateCommunityModal onClose={() => setShowCreateCommunity(false)} onCreated={handleCommunityCreated} />}
    </>
  )
}
