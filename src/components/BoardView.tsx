      ...
      if (filters.types.length > 0) {
        result = result.filter((c) => {
          const type = getDisplayType(
            c.media_type,
            c.genres.includes('Animation') ? [16] : []
          );
  ...
  const handleAddCard = async () => {
    ...
    try {
      ...
      const genres =
        ...
        mapGenreIds(selectedResult.genre_ids);
      const position = generatePosition(
        cards.filter((c) => c.column_id === addColumn).map((c) => c.position)
      );
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p style={{ color: theme.textMuted }}>
            Create a board or select one from the sidebar
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {members.length > 0 && (
            <div className="flex -space-x-2">
              {members.slice(0, 4).map((m) => (
                <div
                  key={m.id}
                  className="w-8 h-8 rounded-full flex items-center justify-center text-sm border-2 cursor-pointer"
                  style={{
                    backgroundColor: theme.bgTertiary,
                    borderColor:
                      ACCENT_COLORS[m.profile?.accent_color || 'purple']
                        .primary,
                  }}
          ...
          <div className="relative">
            <button
              ...
              style={{
                backgroundColor:
                  filterCount > 0 ? theme.accent.bg : theme.bgSecondary,
                border: `1px solid ${theme.border}`,
                color:
                  filterCount > 0 ? theme.accent.primary : theme.textSecondary,
              }}
            >
          ...
          {activeCard && (
            ...
            >
              <div className="flex gap-3">
                <div className="relative w-14 h-20 flex-shrink-0">
                  <Image
                    src={getImageUrl(activeCard.poster_path)}
      ...
      {showAddModal && selectedResult && (
        ...
        >
          ...
          >
                    ...
                    style={{
                      backgroundColor:
                        addColumn === col.id
                          ? theme.accent.bg
                          : theme.bgTertiary,
                      border: `2px solid ${addColumn === col.id ? theme.accent.primary : 'transparent'}`,
                      color:
                        addColumn === col.id
                          ? theme.accent.primary
                          : theme.textSecondary,
            ...
            <label
              className="flex items-center gap-3 mb-6 p-3 rounded-xl cursor-pointer"
              style={{ backgroundColor: theme.bgTertiary }}
            >
              <input
                type="checkbox"
              ...
              <div>
                ...
                <div className="text-xs" style={{ color: theme.textMuted }}>
                  {addPrivate
                    ? "Won't appear in community feeds"
                    : 'Will appear in community feeds'}
                </div>
              </div>
            </label>
            <div className="flex gap-3">
            ...
            <div className="px-6 pb-6 -mt-16">
              <div className="flex gap-4">
                <div className="relative w-24 h-36 flex-shrink-0 rounded-lg overflow-hidden shadow-lg">
                  <Image
                    src={getImageUrl(showCardInfo.poster_path, 'w342')}
                    alt={showCardInfo.title}
                    fill
                  ...
                  <div className="flex gap-2 mt-2 flex-wrap">
                    {showCardInfo.genres.slice(0, 3).map((g) => (
                      <span
                        ...
                        style={{
                          backgroundColor: getGenreStyle(g, darkMode).bg,
                          color: getGenreStyle(g, darkMode).text,
                        }}
                      >
                        {g}
                      </span>
