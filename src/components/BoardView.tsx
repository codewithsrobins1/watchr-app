  ...
  return (
    <div className="flex-1 p-4 lg:p-6 overflow-auto">
      ...
      {showAddModal && selectedResult && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-2 sm:p-4" onClick={() => setShowAddModal(false)}>
          <div className="w-full max-w-md rounded-2xl p-6 max-h-[85vh] overflow-auto" style={{ backgroundColor: theme.bgSecondary, border: `1px solid ${theme.border}` }} onClick={e => e.stopPropagation()}>
            <div className="flex justify-between mb-4">
              <h2 className="text-xl font-bold" style={{ color: theme.text }}>Add to Board</h2>
              <button onClick={() => setShowAddModal(false)} className="p-2.5 rounded-full bg-red-500 hover:bg-red-600 text-white"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {COLUMNS.map(col => (
                  <button key={col.id} onClick={() => setAddColumn(col.id)} className="flex items-center gap-2 px-3 py-3 rounded-lg transition-all"
            ...
            <label className="flex items-center gap-3 mb-6 p-4 rounded-xl cursor-pointer" style={{ backgroundColor: theme.bgTertiary }}>
              <input type="checkbox" checked={addPrivate} onChange={e => setAddPrivate(e.target.checked)} className="w-5 h-5 rounded" />
              <div>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowAddModal(false)} className="flex-1 py-4 rounded-xl font-medium text-base" style={{ backgroundColor: theme.bgTertiary, color: theme.textSecondary }}>Cancel</button>
              <button onClick={handleAddCard} disabled={addLoading} className="flex-1 py-4 rounded-xl font-medium text-white disabled:opacity-50 flex items-center justify-center gap-2 text-base" style={{ backgroundColor: theme.accent.primary }}>
                {addLoading ? <><Loader2 className="w-5 h-5 spinner" />Adding...</> : 'Add to Board'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showReview && reviewCard && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-2 sm:p-4" onClick={() => { setShowReview(false); setReviewCard(null) }}>
          <div className="w-full max-w-md rounded-2xl p-6 max-h-[85vh] overflow-auto" style={{ backgroundColor: theme.bgSecondary }} onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setShowReview(false); setReviewCard(null) }} className="flex-1 py-4 rounded-xl font-medium text-base" style={{ backgroundColor: theme.bgTertiary, color: theme.textSecondary }}>Cancel</button>
              <button onClick={handleReviewSubmit} disabled={!reviewRating} className="flex-1 py-4 rounded-xl font-medium text-white disabled:opacity-50 text-base" style={{ backgroundColor: theme.accent.primary }}>Save Rating</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showCardInfo && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-2 sm:p-4" onClick={() => setShowCardInfo(null)}>
          <div className="w-full max-w-lg rounded-2xl overflow-hidden max-h-[85vh] overflow-auto" style={{ backgroundColor: theme.bgSecondary }} onClick={e => e.stopPropagation()}>
            <div className="relative h-32" style={{ background: `linear-gradient(to bottom, ${theme.accent.primary}40, transparent)` }}>
              <button onClick={() => setShowCardInfo(null)} className="absolute top-4 right-4 p-2.5 rounded-full bg-red-500 hover:bg-red-600 text-white"><X className="w-5 h-5" /></button>
            </div>
