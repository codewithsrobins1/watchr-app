...
export default function SignupPage() {
  ...
  const { isValid: passwordValid, requirements } = useMemo(() => validatePassword(password), [password])
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: theme.bg }}>
      <div className="w-full max-w-md">
        ...
        >
          <h2 className="text-2xl font-bold mb-6" style={{ color: theme.text }}>Create account</h2>
            ...
            <div>
              ...
              <div className="mt-3 space-y-1.5">
                {requirements.map((req, i) => (
                  <div key={i} className="flex items-center gap-2 text-sm transition-colors" style={{ color: req.met ? '#10b981' : theme.textMuted }}>
          ...
          <p className="mt-6 text-center text-sm" style={{ color: theme.textMuted }}>
            ...
            <Link href="/login" className="font-semibold hover:underline" style={{ color: theme.accent.primary }}>
              Sign in
            </Link>
          </p>
        </div>
