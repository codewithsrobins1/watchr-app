...
export default function HomePage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6" style={{ backgroundColor: theme.bg }}>
        <div className="text-center max-w-md">
          ...
          <h1 className="text-xl font-bold mb-2" style={{ color: theme.text }}>Something went wrong</h1>
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: theme.bg }}>
      ...
      {/* Sidebar */}
      <div className={`sidebar-mobile md:relative md:transform-none ${sidebarOpen ? 'open' : ''}`}>
        <Sidebar
          currentView={currentView}
          setCurrentView={(view) => { setCurrentView(view); setSidebarOpen(false) }}
          selectedBoardId={selectedBoardId}
          setSelectedBoardId={(id) => { setSelectedBoardId(id); setCurrentView('board'); setSidebarOpen(false) }}
          selectedCommunityId={selectedCommunityId}
          setSelectedCommunityId={(id) => { setSelectedCommunityId(id); setCurrentView('community'); setSidebarOpen(false) }}
        />
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          ...
          <span className="font-bold" style={{ color: theme.accent.primary }}>Watchr</span>
        </div>
